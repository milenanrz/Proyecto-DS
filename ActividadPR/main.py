...
from db_photographers_operations import *
from db_photographers_connection import AsyncSessionLocal, get_db_session, get_engine
from typing import Annotated
from sqlalchemy.ext.asyncio import AsyncSession
from db_styles_operations import *
from db_styles_connection import AsyncSessionLocal, get_db_session_, get_engine
...
async def show_photographer_db(photographer_id:int, db_session:Annotated[AsyncSession,Depends(get_db_session)]):
    photographer = await db_show_one_photographer(photographer_id = photographer_id, db_session = db_session)
...
async def show_all_photographer_db(db_session:Annotated[AsyncSession,Depends(get_db_session)]):
    photographers =await db_show_all_photographers(db_session=db_session)
...
async def update_photographer_db(db_session:Annotated[AsyncSession,Depends(get_db_session)], photographer_id:int, actual_state:bool):
...
async def delete_photographer_db(photographer_id:int, db_session:Annotated[AsyncSession,Depends(get_db_session)]):
    result = await db_remove_photographer(photographer_id=photographer_id, db_session=db_session)
    if result is None:
        raise HTTPException(status_code=404, detail="Photographer not found")
    return result
...
async def add_style(style:PhotographicStyle, db_session:Annotated[AsyncSession,Depends(get_db_session_)]):
    style_id = await db_add_style(db_session, style.style_name,)
    return {"New style":style_id}

@app.get("/photographic_style/{style_id}", response_model=StyleWithId)
async def show_style_db(style_id:int, db_session:Annotated[AsyncSession,Depends(get_db_session_)]):
    style = await db_show_one_style(style_id = style_id, db_session = db_session)
    if style is None:
...
async def show_all_styles_db(db_session:Annotated[AsyncSession,Depends(get_db_session_)]):
    styles =await db_show_all_styles(db_session=db_session)
    if styles is None:
        raise HTTPException(status_code=404, detail="Photographic styles not found")
    return styles

@app.put("/photographic_style/{style_id}")
async def update_style_db(db_session:Annotated[AsyncSession,Depends(get_db_session_)], style_id:int, new_style:str):
    style = await db_modify_style(style_id=style_id, new_style=new_style, db_session=db_session)
    if style is None:
        raise HTTPException(status_code=404, detail="Photographic style not found")
    return style

@app.delete("/photographic_style/{style_id}")
async def delete_style_db(style_id:int, db_session:Annotated[AsyncSession,Depends(get_db_session_)]):
    result = await db_remove_style(style_id=style_id, db_session=db_session)
    if result is None:
        raise HTTPException(status_code=404, detail="Photographic style not found")
    return result
