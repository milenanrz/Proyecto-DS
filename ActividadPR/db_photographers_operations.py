...
#mostrar por id
async def db_show_one(photographer_id:int, db_session:AsyncSession):
...
async def db_show_all(db_session:AsyncSession): #db_session trae la base de datos
...
#eliminar registro
async def db_remove(photographer_id:int, db_session:AsyncSession):
